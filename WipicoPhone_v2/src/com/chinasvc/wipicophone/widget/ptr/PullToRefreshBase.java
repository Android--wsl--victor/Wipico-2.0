 ...
 *******************************************************************************/
package com.chinasvc.wipicophone.widget.ptr;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			// If we're refreshing, and the flag is set. Eat all
			// MOVE events
			...
			if (isReadyForPull()) {
				...
				// We need to use the correct values, based on
				// scroll
				// direction
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			if (mIsBeingDragged) {
				...
				// If we're already refreshing, just scroll back
				// to the top
				if (isRefreshing()) {
					smoothScrollTo(0);
					return true;
				...
				// If we haven't returned by here, then we're
				// not in a state
	 ...
	 * @deprecated This simple calls setMode with an appropriate mode based
	 *             on the passed value.
	 */
	 ...
	 *         {@link Orientation#HORIZONTAL} depending on the scroll
	 *         direction.
	 */
	...
	/**
	 * This is implemented by derived classes to return the created View. If
	 * you need to use a custom View (such as a custom ListView), override
	 * this method and return an instance of your custom class.
	 * <p/>
	 * Be sure to set the ID of the view in this method, especially if
	 * you're using a ListActivity or ListFragment.
	 * 
	 * @param context
	 *                Context to create view with
	 * @param attrs
	 *                AttributeSet from wrapped class. Means that anything
	 *                you include in the XML layout declaration will be
	 ...
	 * 
	 * @return true if the View is currently in the correct state (for
	 *         example, bottom of a ListView)
	 */
	...
	/**
	 * Called by {@link #onRestoreInstanceState(Parcelable)} so that
	...
	/**
	 * Called by {@link #onSaveInstanceState()} so that derivative classes
	 * can save their instance state.
	 * 
	 * @param saveState
	...
	protected void onRefreshing(final boolean doScroll) {
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				// Call Refresh Listener when the Scroll has
				// finished
			...
			// We're not scrolling, so just call Refresh Listener
			// now
			callRefreshListener();
		}
	}
	...
	protected final Parcelable onSaveInstanceState() {
		...
		// Let derivative classes get a chance to save state first, that
		// way we
	...
	protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		/**
		 * As we're currently in a Layout Pass, we need to schedule
		 * another one to layout any changes we've made here
		 */
		post(new Runnable() {
			@Override
			public void run() {
	...
	protected final void refreshRefreshableViewSize(int width, int height) {
		// We need to set the Height of the Refreshable View to the same
		// as
		// this layout
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			 ...
			 * scrolled at all. We don't use them on the
			 * Header/Footer Views as they change often, which would
			 * negate any HW layer performance boost.
			 */
	...
	/**
	 * Updates the View State when the mode has been set. This does not do
	 * any checking that the mode is different to current state so always
	 * updates.
	 */
	protected void updateUIForMode() {
		// We need to use the correct LayoutParam values, based on
		// scroll
		// direction
		final LinearLayout.LayoutParams lp = getLoadingLayoutLayoutParams();

		// Remove Header, and then add Header Loading View again if
		// needed
		...
		// Remove Footer, and then add Footer Loading View again if
		// needed
		...
		// If we're not using Mode.BOTH, set mCurrentMode to mMode,
		// otherwise
		// set it to pull down
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Refreshable View
		// By passing the attrs, we can add ListView/GridView params via
		// XML
		...
		// Let the derivative classes have a go at handling attributes,
		// then
		// recycle them...
	 ...
	 * 
	 * @return true if the Event has been handled, false if there has been
	 *         no change
	 */
	...
	private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis, OnSmoothScrollFinishedListener listener) {
		...
		if (oldScrollValue != newScrollValue) {
			if (null == mScrollAnimationInterpolator) {
				// Default interpolator is a Decelerate
				// Interpolator
	...
	public static enum AnimationStyle {
		/**
		 * This is the default for Android-PullToRefresh. Allows you to
		 * use any drawable, which is automatically rotated and used as
		 * a Progress Bar.
		 */
		ROTATE,
		 ...
		 * Uses an arrow image which flips depending on where the user
		 * has scrolled.
		 */
		FLIP;
		 ...
		 * state, or inflating the view from XML where the mode is given
		 * through a attr int.
		 * 
		 * @param modeInt
	...
	public static enum Mode {
		...
		/**
		 * Only allow the user to Pull from the start of the Refreshable
		 * View to refresh. The start is either the Top or Left,
		 * depending on the scrolling direction.
		 */
		PULL_FROM_START(0x1),

		/**
		 * Only allow the user to Pull from the end of the Refreshable
		 * View to refresh. The start is either the Bottom or Right,
		 * depending on the scrolling direction.
		 */
		PULL_FROM_END(0x2),

		/**
		 * Allow the user to both Pull from the start, from the end to
		 * refresh.
		 */
		BOTH(0x3),

		/**
		 * Disables Pull-to-Refresh gesture handling, but allows
		 * manually setting the Refresh state via
		 ...
		 * @param modeInt
		 *                - int to map a Mode to
		 * @return Mode that modeInt maps to, or PULL_FROM_START by
		 *         default.
		 */
		...
		/**
		 * @return true if this mode wants the Loading Layout Header to
		 *         be shown
		 */
		...
		/**
		 * @return true if this mode wants the Loading Layout Footer to
		 *         be shown
		 */
	...
	/**
	 * Simple Listener that allows you to be notified when the user has
	 * scrolled to the end of the AdapterView. See (
	 ...
	 * finished a touch event. Useful when you want to append extra UI
	 * events (such as sounds). See (
	...
	public static interface OnPullEventListener<V extends View> {
		 ...
		 *                {@link Mode#PULL_FROM_END} depending on which
		 *                direction the user is pulling. Only useful
		 *                when <var>state</var> is
	...
	public static interface OnRefreshListener<V extends View> {
		...
		/**
		 * onRefresh will be called for both a Pull from start, and Pull
		 * from end
		 */
	...
	/**
	 * An advanced version of the Listener to listen for callbacks to
	 * Refresh. This listener is different as it allows you to differentiate
	 * between Pull Ups, and Pull Downs.
	 * 
	 * @author Chris Banes
	 */
	public static interface OnRefreshListener2<V extends View> {
		// TODO These methods need renaming to START/END rather than
		// DOWN/UP

		/**
		 * onPullDownToRefresh will be called only when the user has
		 * Pulled from the start, and released.
		 */
		public void onPullDownToRefresh(final PullToRefreshBase<V> refreshView);

		/**
		 * onPullUpToRefresh will be called only when the user has
		 * Pulled from the end, and released.
		 */
	...
	public static enum State {
		...
		/**
		 * When the UI is in a state which means that user is not
		 * interacting with the Pull-to-Refresh function.
		 */
		RESET(0x0),

		/**
		 * When the UI is being pulled by the user, but has not been
		 * pulled far enough so that it refreshes when released.
		 */
		PULL_TO_REFRESH(0x1),

		/**
		 * When the UI is being pulled by the user, and
		 * <strong>has</strong> been pulled far enough so that it will
		 * refresh when released.
		 */
		RELEASE_TO_REFRESH(0x2),

		/**
		 * When the UI is currently refreshing, caused by a pull
		 * gesture.
		 */
		REFRESHING(0x8),
		...
		/**
		 * When the UI is currently overscrolling, caused by a fling on
		 * the Refreshable View.
		 */
		OVERSCROLLING(0x10);

		/**
		 * Maps an int to a specific state. This is needed when saving
		 * state.
		 * 
		 * @param stateInt
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			/**
			 * Only set mStartTime if this is the first time we're
			 * starting, else actually calculate the Y delta
			 */
			if (mStartTime == -1) {
				...
				/**
				 * We do do all calculations in long to reduce
				 * software float calculations. We use 1000 as
				 * it gives us good accuracy and small rounding
				 * errors
				 */
