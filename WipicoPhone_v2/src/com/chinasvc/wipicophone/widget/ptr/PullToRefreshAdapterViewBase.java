...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements OnScrollListener {
	...
	/**
	 * Gets whether an indicator graphic should be displayed when the View
	 * is in a state where a Pull-to-Refresh can happen. An example of this
	 * state is when the Adapter View is scrolled to the top and the mode is
	 * set to {@link Mode#PULL_FROM_START}. The default value is
	 * <var>true</var> if
	...
	public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount, final int totalItemCount) {
		...
		/**
		 * Set whether the Last Item is Visible. lastVisibleItemIndex is
		 * a zero-based index, so we minus one totalItemCount to check
		 */
	...
	public final void onScrollStateChanged(final AbsListView view, final int state) {
		/**
		 * Check that the scrolling has stopped, and that the last item
		 * is visible.
		 */
	 ...
	 * {@link PullToRefreshBase#getRefreshableView() getRefreshableView()}.
	 ...
	 * <p/>
	 * We need it handle it ourselves so that we can Pull-to-Refresh when
	 * the Empty View is shown.
	 * <p/>
	...
	public final void setEmptyView(View newEmptyView) {
		...
		if (null != newEmptyView) {
			// New view needs to be clickable so that Android
			// recognizes it as a
			...
			// We need to convert any LayoutParams so that it works
			// in our
			// FrameLayout
	...
	/**
	 * Sets whether an indicator graphic should be displayed when the View
	 * is in a state where a Pull-to-Refresh can happen. An example of this
	 * state is when the Adapter View is scrolled to the top and the mode is
	 * set to {@link Mode#PULL_FROM_START}
	 * 
	 * @param showIndicator
	...
	private void addIndicatorViews() {
		...
		if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			// If the mode can pull down, and we don't have one set
			// already
	...
	private boolean isFirstItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			 ...
			 * PtRListView internally use a HeaderView which messes
			 * the positions up. For now we'll just add one to
			 * account for it and rely on the inner condition which
			 * checks getTop().
			 */
	...
	private boolean isLastItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			 ...
			 * == lastItemPosition, but PtRListView internally uses
			 * a FooterView which messes the positions up. For me
			 * we'll just subtract one to account for it and rely on
			 * the inner condition which checks getBottom().
			 */
