...
public class ViewFlow extends AdapterView<Adapter> {
	...
	public static interface ViewSwitchListener {
		 ...
		 * @param position
		 *                The position in the adapter of the
		 *                {@link View} currently in focus.
		 */
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (xMoved) {
				// Scroll if the user moved far enough along the
				// X axis
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			/*
			 * The actual horizontal scroll origin does typically
			 * not match the perceived one. Therefore, we need to
			 * calculate the perceived horizontal scroll origin
			 * here, since we use a view buffer.
			 */
