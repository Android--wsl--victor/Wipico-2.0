 ...
 */
@SuppressLint({ "FieldGetter", "HandlerLeak" })
public class WheelView extends View {
	...
	private static final int VALUE_TEXT_COLOR = 0xFFbbbbbb;
	 ...
	 * 通知监听器已经开始了滚动
	 */
	protected void notifyScrollingListenersAboutStart(float direction) {
		for (OnWheelScrollListener listener : scrollingListeners) {
			listener.onScrolling(this, direction);
		}
	}
	...
	/**
	 * 设置当前Item的值，如果索引错误，则不进行任何操作
	 * 
	 * @param index
	 *                值的索引
	 * @param animated
	 *                动画标记the animation flag
	 */
	...
	/**
	 * Sets the current item w/o animation. Does nothing when index is
	 * wrong.
	 * 
	 * @param index
	 *                the item index
	...
	private void initResourcesIfNecessary() {
		...
		setBackgroundResource(R.drawable.bg_mouse_middle);
	 ...
	 * Starts scrolling
	 */
	private void startScrolling(float direction) {
		// if (!isScrollingPerformed) {
		// isScrollingPerformed = true;
		// notifyScrollingListenersAboutStart(direction);
		// }
		notifyScrollingListenersAboutStart(direction);
	}
	...
	public void scroll(int itemsToScroll, int time) {
		...
		setNextMessage(MESSAGE_SCROLL);
		Log.i("TTT", "itemsToScroll>>>>" + itemsToScroll);
		startScrolling(itemsToScroll);
	}
}
