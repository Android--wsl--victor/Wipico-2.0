...
public class ImagePagerAdapter extends PagerAdapter {
	...
	public Object instantiateItem(View view, final int position) {
		...
		if (listDatas.get(position).getUrl().startsWith("assets")) {
			imageUrl = listDatas.get(position).getUrl();
		} else {
			imageUrl = "file://" + listDatas.get(position).getUrl();
		}
		...
		imageLoader.displayImage(imageUrl, imageView, options, new SimpleImageLoadingListener() {
			...
			public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
				listDatas.get(position).setUrl("assets://image_default.png");
				imageLoader.displayImage(listDatas.get(position).getUrl(), imageView);
