...
public abstract class UDPListener extends Listener {
	...
	// TODO 由子类来赋初始值
	private int port = Config.MESSAGE_PORT;
	...
	protected void send(final String msg, final InetAddress destIp, final int destPort) {
		executor.execute(new Runnable() {
			...
			public void run() {
				try {
					DatagramPacket packet = new DatagramPacket(msg.getBytes(Config.ENCOD), msg.length(), destIp, destPort);
					socket.send(packet);
					if (!isOnline)
						socket.close();
				} catch (IOException e) {
