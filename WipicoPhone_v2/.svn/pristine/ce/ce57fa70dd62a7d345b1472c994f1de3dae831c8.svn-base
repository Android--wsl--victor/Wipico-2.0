...
public class RemoteImageActivity extends ControlBaseActivity implements OnItemClickListener {
		...
		if (!isLoading) {
			if (name == null) {
				if (MainActivity.mWipicoFileClient != null) {
					...
					Toast.makeText(RemoteImageActivity.this, R.string.msg_no_ftp, Toast.LENGTH_SHORT).show();
				}
			} else {
				if (MainActivity.mWipicoFileClient != null) {
					if (isOnResume) {
		...
		protected Boolean doInBackground(Void... arg0) {
			...
			if (MainActivity.mWipicoFileClient != null) {
				if (serverPath.equals(FTP_ROOT_FILE)) {
					...
					listServerDatas = MainActivity.mWipicoFileClient.getImageList(serverPath);
				}
				return true;
			} else {
				return false;
		...
		popListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view, int position, long id) {
				...
				if (listPopDatas.get(position).getType() == PopType.LOCAL_PLAY) {
					...
					listShares.add(file);
					((ImageTabActivity) getParent()).mainActivity.showShareView(listShares, 1);
