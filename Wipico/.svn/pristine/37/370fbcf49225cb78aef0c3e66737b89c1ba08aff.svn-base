	...
	private void procEvent(byte[] data, String ip) {
		...
		try {
			...
			case WipicoConstant.WIFI_CONNECT:
				...
				break;
			case WipicoConstant.SET_WIFI:
				if (mOperateListener != null) {
					mOperateListener.setWifi(is.readUTF(), is.readUTF(), is.readUTF());
				}
				break;
			case WipicoConstant.WIFI_FORGET_CONNECT:
				...
				break;
			case WipicoConstant.GET_WIFI_LIST:
				...
				try {
					...
					SyncPlayer.sendWifiList(json, ip);
